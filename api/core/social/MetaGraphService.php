...
class MetaGraphService {
    ...
    /**
     * Publish activity to selected platforms (Facebook / Instagram)
     */
    public function publishActivity(string $activityId, string $title, string $description, ?string $imageUrl, bool $toFacebook, bool $toInstagram): array {
        $results = [];

        $cleanDesc = trim(preg_replace('/\s+/', ' ', strip_tags($description)));
        if (mb_strlen($cleanDesc) > 350) {
        ...
        if ($toInstagram) {
            $results['instagram'] = $this->publishActivityToInstagram($activityId, $message, $imageUrl);
        }
    ...
    /**
     * Publish activity to Facebook Page
     */
    public function publishActivityToFacebook(string $activityId, string $message, ?string $imageUrl): array {
        if (!$this->isConfigured()) {
            $error = 'Meta Graph API no configurada (falta META_PAGE_ID o META_PAGE_ACCESS_TOKEN).';
            $this->recordActivitySocialLink($activityId, 'facebook', 'unconfigured', null, 'failed', $error);
            return ['status' => 'failed', 'error' => $error];
        }
        ...
        try {
            ...
            if (isset($response['id'])) {
                $externalId = (string)$response['id'];
                $permalink = "https://www.facebook.com/{$externalId}";
                $this->recordActivitySocialLink($activityId, 'facebook', $externalId, $permalink, 'synced', null);
                return ['status' => 'synced', 'external_post_id' => $externalId, 'permalink' => $permalink];
            }

            $errMsg = $response['error']['message'] ?? 'Error desconocido al publicar actividad en Facebook.';
            $this->recordActivitySocialLink($activityId, 'facebook', 'err_' . time(), null, 'failed', $errMsg);
            return ['status' => 'failed', 'error' => $errMsg];

        } catch (Throwable $e) {
            $this->recordActivitySocialLink($activityId, 'facebook', 'err_' . time(), null, 'failed', $e->getMessage());
            return ['status' => 'failed', 'error' => $e->getMessage()];
        }
    }

    /**
     * Publish activity to Instagram Business
     */
    public function publishActivityToInstagram(string $activityId, string $message, ?string $imageUrl): array {
        $igUserId = $this->metaConfig['ig_user_id'] ?? '';
        $token = $this->metaConfig['page_access_token'] ?? '';
        $version = $this->metaConfig['api_version'] ?? 'v21.0';
        ...
        if (empty($igUserId) || empty($token)) {
            $error = 'Instagram Business ID no configurado (falta META_IG_USER_ID o token).';
            $this->recordActivitySocialLink($activityId, 'instagram', 'unconfigured', null, 'failed', $error);
            return ['status' => 'failed', 'error' => $error];
        }

        if (empty($imageUrl) || !filter_var($imageUrl, FILTER_VALIDATE_URL)) {
            $error = 'Instagram requiere obligatoriamente una imagen con URL pública válida (http/https).';
            $this->recordActivitySocialLink($activityId, 'instagram', 'invalid_media', null, 'failed', $error);
            return ['status' => 'failed', 'error' => $error];
        }
        ...
        try {
            ...
            if (!isset($containerRes['id'])) {
                $errMsg = $containerRes['error']['message'] ?? 'Error al crear contenedor de media en Instagram.';
                $this->recordActivitySocialLink($activityId, 'instagram', 'err_' . time(), null, 'failed', $errMsg);
                return ['status' => 'failed', 'error' => $errMsg];
            }
            ...
            if (isset($publishRes['id'])) {
                $externalId = (string)$publishRes['id'];
                $permalink = "https://www.instagram.com/p/{$externalId}/";
                $this->recordActivitySocialLink($activityId, 'instagram', $externalId, $permalink, 'synced', null);
                return ['status' => 'synced', 'external_post_id' => $externalId, 'permalink' => $permalink];
            }
            ...
            $this->recordActivitySocialLink($activityId, 'instagram', 'err_' . time(), null, 'failed', $e->getMessage());
            return ['status' => 'failed', 'error' => $e->getMessage()];
        }
    }
    ...
    /**
     * Record or update activity social link status in the database
     */
    public function recordActivitySocialLink(string $activityId, string $platform, string $externalId, ?string $permalink, string $syncStatus, ?string $syncError): void {
        $stmt = $this->pdo->prepare('
            INSERT INTO activity_social_links (activity_id, platform, external_post_id, external_permalink, sync_status, sync_error, synced_at)
            VALUES (:activity_id, :platform, :external_post_id, :permalink, :sync_status, :sync_error, :synced_at)
            ON DUPLICATE KEY UPDATE
                external_post_id   = VALUES(external_post_id),
                external_permalink = VALUES(external_permalink),
                sync_status        = VALUES(sync_status),
