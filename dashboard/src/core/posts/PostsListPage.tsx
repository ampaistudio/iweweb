  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-white">Novedades y Blog</h2>
          ...
          <div className="space-y-3">
            {posts.map((post) => {
              ...
              return (
                <div
                  key={post.id}
                  className="p-4 sm:p-5 rounded-2xl bg-stone-950/80 border border-stone-800 hover:border-stone-700/80 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4"
                >
                  <div className="flex items-start gap-4">
                    ...
                    {post.cover_image_url ? (
                      ...
                      <div className="w-16 h-16 rounded-xl bg-stone-900 border border-stone-800 flex items-center justify-center text-xl flex-shrink-0 text-stone-500">
                        📄
                      </div>
                    )}
                    ...
                    <div className="space-y-1.5">
                      <div className="flex flex-wrap items-center gap-2">
                        <h4 className="text-sm font-bold text-stone-100">{post.title}</h4>
                        <Badge variant={isPublished ? 'success' : 'warning'} size="sm">
                          {isPublished ? 'Publicado' : 'Borrador'}
                        </Badge>
                        {!isWeb && (
                          <span className="text-[11px] bg-sky-950 text-sky-300 border border-sky-800 px-2 py-0.5 rounded-md font-medium">
                  ...
                  {/* Actions */}
                  <div className="flex items-center gap-2 self-end md:self-center pt-2 md:pt-0 border-t md:border-t-0 border-stone-800 w-full md:w-auto justify-end">
